...
export default function TagClient({ articles, tag }: { articles: any[], tag: string }) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-12">
        <div className="flex flex-col items-center text-center mb-16 space-y-4">
          ...
          <h1 className="text-5xl md:text-7xl font-headline font-black tracking-tight capitalize">{tag}</h1>
          <p className="text-muted-foreground text-xl max-w-2xl font-body italic">
            Članci i vijesti označeni ovom oznakom.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-6 justify-end items-center mb-12">
          <div className="relative w-full md:w-80">
            ...
            <Input 
              placeholder="Pretraži unutar oznake..." 
            ...
            <Link key={article.id} href={getLocalizedUrl(`/blog/${article.slug || article.id}`, language)}>
              <Card className="group overflow-hidden border-none shadow-xl hover:shadow-2xl transition-all duration-500 rounded-[2.5rem] bg-white/60 backdrop-blur-md h-full flex flex-col">
                <div className="relative h-64 overflow-hidden bg-slate-100 flex items-center justify-center">
                  ...
                  <Badge className="absolute top-6 left-6 bg-white/95 text-primary border-none shadow-lg font-black">{article.category || 'Blog'}</Badge>
                </div>
        ...
        {filteredArticles.length === 0 && (
          <div className="text-center py-24">
            <h3 className="text-2xl font-bold text-muted-foreground">Trenutno nema članaka s ovom oznakom.</h3>
            <Button variant="link" onClick={() => {setSearchQuery('')}}>Poništi pretragu</Button>
          </div>
        )}
      </main>
    </div>
