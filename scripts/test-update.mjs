...
async function run() {
  try {
    const records = await pb.collection('listings').getList(1, 1, { filter: 'name="Opća bolnica Dubrovnik"' });
    const record = records.items[0];
    await pb.collection('listings').update(record.id, { photoUrls: record.photoUrls });
  } catch (e) {
